...
#чистая функция, функции не используют глобальные переменные, не изменяют входные данные, легко тестируются
reverse_num = lambda x: int(str(abs(x))[::-1]) * (-1 if x < 0 else 1)

#чистая функция
count_common_with_reverse = lambda a, b: sum(
    map(
        lambda x: (x in b) or (reverse_num(x) in b),
        a
    )
)
    ...
    while True:
        ...
        if state == "NO_DATA":
            ...
            actions = {
                ...
                "4": lambda: print("Результат:", result if result is not None else msgs["no_result"])
            }
